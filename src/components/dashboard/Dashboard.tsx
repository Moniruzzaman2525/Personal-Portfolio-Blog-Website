...
import { Menu, LayoutDashboard, BarChart3, Settings, MessageCircle, User, LogOut, ChevronDown, ChevronUp, FilePlus2, List } from "lucide-react";
        ...
        <div className="flex min-h-screen bg-gray-100">
            <aside className={`fixed md:relative h-screen bg-white shadow-lg transition-all duration-300 p-4 z-50 md:w-64 w-16 ${isMobileMenuOpen ? "block" : "hidden"} md:block`}>
                ...
                <nav className="mt-6 space-y-4">
                    ...
                    <button onClick={() => setIsBlogsOpen(!isBlogsOpen)} className="flex justify-between items-center w-full p-2 rounded-lg hover:bg-gray-200 cursor-pointer">
                        <div className="flex items-center space-x-2">
                            <BarChart3 className="h-5 w-5" />
                            <span className="hidden md:block font-medium">Blog Management</span>
                        </div>
                        {isBlogsOpen ? <ChevronUp className="h-5 w-5" /> : <ChevronDown className="h-5 w-5" />}
                    </button>
                    {isBlogsOpen && (
                    ...
                    <button onClick={() => setIsProjectsOpen(!isProjectsOpen)} className="flex justify-between items-center w-full p-2 rounded-lg hover:bg-gray-200 cursor-pointer">
                        <div className="flex items-center space-x-2">
                            ...
                            <span className="hidden md:block font-medium">Project Management</span>
                        </div>
                        {isProjectsOpen ? <ChevronUp className="h-5 w-5" /> : <ChevronDown className="h-5 w-5" />}
                    </button>
                    {isProjectsOpen && (
                        <div className="ml-6 space-y-2">
            ...
            <div className="flex flex-col flex-1">
                <nav className="bg-white shadow-md px-6 py-4 flex justify-between items-center fixed w-full top-0 left-0 right-0 z-40">
                    <button className="md:hidden p-2 rounded-lg bg-gray-200 hover:bg-gray-300" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
                    ...
                    <div className="flex items-center space-x-4">
                        ...
                        <button onClick={() => signOut()} className="flex items-center space-x-2 p-2 rounded-lg bg-red-500 text-white hover:bg-red-600">
